...
Chord.prototype.getNodeFor = function (value, callback) {
  ...
  // The fact that we do not know our predecessor implies that a join is in
  // progress. This *may* mean that our yet (unknown) predecessor does also not
  // know us. Hence the successor pointers are not all correct yet. That may
  // lead to incorrect results, because we as well as our future predecessor
  // feel responsible for the same interval. To avoid this, if we do not know
  // our predecessor, we deny to answer and requests and instead forward the
  // requests to our successor.
  if (!this.predecessor) {
    target.host = this.successor.host;
    target.port = this.successor.port;
  }

  remote(target.host, target.port).run('find-successor', {
    id: getId(value)
